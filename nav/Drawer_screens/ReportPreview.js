...
class ReportPreview extends Component {
  ...
  async componentDidMount() {
    ...
    console.log("ReportPreview - " + JSON.stringify(this.props))
    this.getData(this.props.route.params.reportNumber)
    console.log("ReportPreview - " + JSON.stringify(userObj));
  ...
  getData = async (reportNumber) => {
    ...
    console.log("ReportPreview Number" + reportNumber);
    try {
      let res = await fetch(
        "https://srobvq5uw2.execute-api.us-west-2.amazonaws.com/dev/users/reports/" + reportNumber,
        {
          method: "GET",
          headers,
        }
      ...
      res = await res.json();
      console.log("ReportPreview Response JSON" + JSON.stringify(res));
  ...
  render() {
    const getHeader = () => {
      return (
        <React.Fragment>
          ...
          <Text style={styles.value}>{this.formData.reportType}</Text>
          <Text style={styles.header}>Visit Date</Text>
          <Text style={styles.value}>{this.formData.visitDate}</Text> */}
    ...
    console.log("ReportPreiview - Pre Render Formdata - " + JSON.stringify(this.formData))
    return (

      <SafeAreaView style={styles.container}>
        {!this.formData.userId ? <></> : <View>
          <FlatList
            data={this.formData.chiefcomplaintSymptoms}
            renderItem={renderChiefItem}
            style={styles.flatlist}
            keyExtractor={(item) => item.recordTime?item.recordTime.toString():1}
...
export default connect(mapStateToProps)(ReportPreview)